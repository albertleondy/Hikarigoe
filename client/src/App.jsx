...
function App() {
  ...
  const downloadAllOpus = () => {
    queuedVideos.forEach((video, index) => {
      setTimeout(() => {
        const iframe = document.createElement('iframe');
        iframe.style.display = 'none';
        iframe.src = `http://localhost:3001/api/ytdl/download?url=${encodeURIComponent(video.url)}&type=opus&embedThumbnail=${queueEmbedThumbnail}`;
  ...
  return (
    <div className="app-container" style={{ display: 'flex', gap: '20px', alignItems: 'stretch' }}>
      ...
      {queuedVideos.length > 0 && currentView === 'youtube-search' && (
        <div className="queue-sidebar glass-card" style={{ width: '320px', flexShrink: 0, padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'stretch', height: '100%', boxSizing: 'border-box' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', display: 'flex', flexDirection: 'column', gap: '10px', paddingRight: '5px', width: '100%' }}>
            {queuedVideos.map(video => (
              <div key={video.id} className="queue-item" style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(255,255,255,0.05)', padding: '10px', borderRadius: '12px', position: 'relative', overflow: 'hidden' }}>
